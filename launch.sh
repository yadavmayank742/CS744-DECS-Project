#!/bin/bash
  ...
  installations need access to root directory !!!"
	...
	*)
       ...
       |                                                                                   |
       |       \033[48;2;255;0;0m\033[1;94m\033[38;2;255;255;255m            <<< C Through >>>            \033[0m       |
       |___________________________________________________________________________________|
...
[Install]
WantedBy=network-online.target" > /etc/systemd/system/c-through.service
		echo " The client for $preset_uname connecting to server @ $server_hostname:$server_port_no will start as service !!"
		systemctl daemon-reload
		systemctl enable c-through
		systemctl start c-through
		;;
	5)
		check_root
		systemctl stop c-through
		systemctl disable c-through
		systemctl daemon-reload
		rm /bin/__client__
		rm /etc/systemd/system/c-through.service
